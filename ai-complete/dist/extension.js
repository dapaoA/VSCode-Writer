...
const RECOMMENDED_EDITOR_SETTINGS = {
    'editor.wordWrap': 'on',
...
function activate(ctx) {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('aiComplete.setDeepSeekApiKey', async () => {
        try {
    ...
    // );
    // 提供给webview获取密钥（内部命令）
    ctx.subscriptions.push(vscode.commands.registerCommand('aiComplete.getOpenAIKey', async () => {
        return await ctx.secrets.get(SECRET_KEY_OPENAI);
    }));
    ctx.subscriptions.push(vscode.commands.registerCommand('aiComplete.getDeepSeekKey', async () => {
        return await ctx.secrets.get(SECRET_KEY_DEEPSEEK);
    }));
    // 开关
    ctx.subscriptions.push(vscode.commands.registerCommand('aiComplete.toggle', async () => {
        const cfg = vscode.workspace.getConfiguration();
        const enabled = cfg.get(`${CFG}.enabled`, true);
        ...
        try {
            if (newEnabled) {
                ...
                // 恢复原值
                const prev = (await ctx.globalState.get(STORAGE_PREV_SETTINGS)) || {};
                for (const key of Object.keys(RECOMMENDED_EDITOR_SETTINGS)) {
    ...
    // 再生当前建议（触发一次新的内联建议）
    ctx.subscriptions.push(vscode.commands.registerCommand('aiComplete.regenerate', async () => {
        try {
    ...
    // 显示对话面板
    ctx.subscriptions.push(vscode.commands.registerCommand('aiComplete.showChatPanel', () => {
        vscode.commands.executeCommand('ai-complete-chat-panel.focus');
    }));
    // 已移除侧栏注册
    ...
    ctx.subscriptions.push(vscode.languages.registerCompletionItemProvider(selector, new CategoryCompletionProvider(ctx)));
    // 在文本变化时重置 quick 模式
...
class WritingInlineProvider {
    ...
    async provideInlineCompletionItems(document, position, context, token) {
        ...
        if (/^\s*$/.test(prevWindow))
            return;
        const maxCtx = vscode.workspace.getConfiguration().get(`${CFG}.maxCharsContext`, 2000);
        const maxOut = vscode.workspace.getConfiguration().get(`${CFG}.maxSuggestionChars`, 140);
...
function getContextBeforePosition(document, position, charCount) {
...
function getContextAfterPosition(document, position, charCount) {
...
async function tryVsCodeLM(prompt, maxOut, token) {
...
async function tryOpenAI(ctx, prompt, maxOut, temperature, token) {
    const model = vscode.workspace.getConfiguration().get(`${CFG}.openai.model`, 'gpt-4o-mini');
...
async function tryDeepSeek(ctx, prompt, maxOut, temperature, token) {
    const model = vscode.workspace.getConfiguration().get(`${CFG}.deepseek.model`, 'deepseek-chat');
...
// === Ollama 本地回退 ===
async function tryOllama(prompt, maxOut, token) {
    const model = vscode.workspace.getConfiguration().get(`${CFG}.ollama.model`, 'qwen2.5:7b');
    try {
...
class CategoryCompletionProvider {
    constructor(ctx) {
        this.ctx = ctx;
    }
...
async function showCategoryCandidates(ctx, digit) {
...
async function showCustomInfill(ctx, requirement) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
    const document = editor.document;
